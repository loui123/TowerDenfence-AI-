...
- **潮林術 (水+木)**：
  - 子彈數量：`6 到 9 發 (隨機)`
  - 每發傷害：`塔基礎傷害 + (波數 * 10)`
  - 附加：每次命中皆會造成**獨立爆擊判定**，並隨機附加一層對應異常狀態（如中毒、流血、凍傷、灼燒、緩速、麻痺、暈眩、擊退）。
- **蒸潮術 (水+火)** 及 **焚森術 (木+火)**：
  - 區域範圍：半徑 1.5 格 (直徑 3 格)
  - 存在時間：持續 3 秒
  - 傷害頻率：每 0.5 秒一跳
  - 傷害值：總和基礎為 `塔基礎傷害 + (波數 * 5)`。
    - **蒸潮術** 會切分為 50% 火傷與 50% 水傷。
    - **焚森術** 會切分為 50% 木傷與 50% 火傷。
  - 附加效果：每跳傷害會對目標附加 2 層中毒與 2 層灼傷，並具備範圍牽引與緩速機制。
...
## 2026-02-28 更新（平衡調整與特效重製）

### 1) 平衡調整
- **暴擊傷害之書**：修正 UI 顯示未加上 `critDmgBookBonus` 的問題。
- **靈氣技能 Nerf**（一般塔三選一專精）：
  - 速度靈氣 (`spec_speed_aura`)：全塔攻速 +10% 降低為 +5%。
  - 暴擊靈氣 (`spec_crit_global`)：全塔暴擊機率 +20% 降低為 +10%。
  - 暴傷靈氣 (`spec_crit_dmg_global`)：全塔暴擊傷害 +20% 降低為 +10%。

### 2) 融合法術與控制機制修復
- **風眼牽引機制重構**：為避免多個龍捲風 / 融合控制場將怪物超高速牽引至地圖外，新增 `pullingEffectIndex` 機制。一隻怪物在同一幀只能被一個牽引源吸引。
- **潮林術異常狀態修正**：修復潮林術投射物命中時因未定義函式（如舊版 `applyPalsy` 被移除）導致的當機問題，並完善流血與麻痺狀態的邏輯。

### 3) 視覺特效重製
- **塔的去背與換色**：
  - 法術塔 (`magic_tower`) 去背並改為深藍/深紫色的神祕水晶塔。
  - 緩速塔 (`slow_tower`) 去背並改為冰晶與紫色的配色。
  - 輔助塔 (`support_tower`) 去背並改為明亮的黃色系配色。
- **水球術動畫翻新**：由原本擴散的水花改為**向目標中心凝聚的碎冰晶**，並且限制其繪圖大小不超過 1.2 格，避免遮擋畫面。
- **新融合法術專屬動畫**：
  - **焚森術**：命中後在地圖上產生 1.5 格半徑（直徑 3 格）的**火焰漩渦 (Fire Vortex)**，持續 3 秒。
  - **蒸潮術**：命中後在地圖上產生 1.5 格半徑的**火山爆發/蒸汽勃發 (Eruption)** 特效，持續 3 秒。
  - **潮林術**：投射物外觀改為順著軌跡飛行的樹葉 (Leaf) 與水滴 (Water Drop)。
